...
{
    /// <summary>
    /// XMLDSig signing/verification for ISO 15118-20 DC — identical suite and shape to
    /// <see cref="Vanaheimr.V2G.Iso15118_20.CommonMessages.V2GSignature"/> (SHA-512 digests, ECDSA-P521
    /// or Ed448 signatures), just against DC's own duplicated <c>SignedInfoType</c>/codec.
    /// </summary>
    public static class V2GSignature
    {
        ...
        public static byte[] SignedInfoFragment(SignedInfoType signedInfo)
        {
            ...
            while (true)
            {
                if (DcCodec.EncodeFragment_SignedInfo(signedInfo, buf, out int n))
