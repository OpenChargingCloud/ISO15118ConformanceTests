...
#
# Usage: sdp-responder.py <iface> <secc-tcp-port>     (e.g. sdp-responder.py eth0 55000)
...
# SDP response payload: 16-byte IPv6 addr, 2-byte port, security=0x10 (NO_TLS), transport=0x00 (TCP)
payload = socket.inet_pton(socket.AF_INET6, lladdr) + struct.pack("!H", port) + bytes([0x10, 0x00])
...
print(f">>> SDP responder ready on {iface} (ff02::1:15118); advertising [{lladdr}%{iface}]:{port}", flush=True)
