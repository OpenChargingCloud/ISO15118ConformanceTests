...
| **Renegotiation, -2** | EVerest | ◐ `EV→` | **Re-run 2026-08-15 with the fix, and the wall moved.** Their station accepts the renegotiated `CableCheckReq` — four `OK`s — where the pre-fix car got `FAILED_SequenceError` three minutes earlier on the same binary; the session then dies inside their `EvseManager`, whose cable check waits for the DC link to drop below 60 V and raises `MREC11CableCheckFault` → `Inoperative` when it does not. **Not filed**: our `CableCheckReq` carries `EVReady = true` and our car drops no demand, so the likelier owner is us again. The deciding arm is named in [the run note](interop-runs/2026-08-15-everest-iso2-renegotiation-rerun/notes.md) and has not been run. |
