...
2 request frame(s), 1 response frame(s).
...
### station → EV
  ...
  -   SessionSetupRes   (in the reference, never answered)
...
**23 divergence(s) in the order.** Each one is a question for the write-up: our state machine, their capture, or a real disagreement?
