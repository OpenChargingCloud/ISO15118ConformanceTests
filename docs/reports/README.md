...
| [`everest-isomux-trusted-ca-keys.md`](everest-isomux-trusted-ca-keys.md) | EVerest (`IsoMux`) | one | The multiplexer asks libevse-security for its certificate through the call that carries `include_root = false`, so it never sets a trust anchor, its chain is never verified or registered, and its TLS server boots with **`trusted_ca_keys support disabled`** — the extension `[V2G2-651]` obliges every EVCC to send and `[V2G2-871]` obliges the station to select on. `EvseV2G`, in the same process 4 ms later, logs neither warning. Invisible with one V2G root; with two the mux serves the first and an EV holding the other walks away |
| [`everest-isomux-continues-after-read-failure.md`](everest-isomux-continues-after-read-failure.md) | EVerest (`IsoMux`) | one | The multiplexer logs *"v2g_incoming_v2gtp() failed"* and then, in the next line, logs *"Handling SupportedAppProtocolReq"* — it decodes the buffer it never read, takes its backend routing decision from that, and proxies to the `-2` module. In the same seven lines the retry loop tests `rv == 1`, the value that means the peer has closed. `EvseV2G`, which it was forked from, has the `goto error_out` it lost. Reproduced with a control that differs by two bytes |
