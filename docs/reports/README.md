...
a report; that file is how you work through them — thirty-four sendable reports, forty-eight issues, six
...
| [`evdriveflow-session-id.md`](evdriveflow-session-id.md) | EDF Lab (eVDriveFlow) | **5** | Their `-20` SECC **never reads the incoming SessionID**: fifteen `process_*_request.py` handlers write their own id into the response header and none compares, so `[V2G20-460]` is unimplemented and any request at all is served as the session owner's. Two of the four `-20`/`-2` stacks audited get it right, one has a narrow defect, this one has none of it. **On their station 2026-08-15, and it went further than the checklist expected**: with `DEADBEEFDEADBEEF` and with eight zero bytes, **ten message types answered `OK`** — `AuthorizationSetup` through **`PowerDelivery`**, the request that closes the contactor — in sessions identical to the control message for message, with their own debug log printing the foreign id three lines above the answer that ignores it. The same probe is **refused** by EVerest's `-20` station, which is what rules out the instrument; the source grep was re-run **inside the container that answered**. Ten of the thirteen handlers the rule applies to — and reaching ten rather than three took teaching our own EV to send a legal `SupportedServiceIDs` filter, since their *other* defect ends every unfiltered session at the fifth message |
| [`evdriveflow-session-id-entropy.md`](evdriveflow-session-id-entropy.md) | EDF Lab (eVDriveFlow) | **6** | `generate_random_session_id()` draws `secrets.randbelow(10⁸)` and zero-pads it to eight ASCII digits, so the 64-bit `SessionID` never leaves a 10⁸-wide corner of its field: **26,6 bits of entropy where `[V2G20-2621]` requires 58**, and a repeat expected after ~10 000 sessions. The generator is right and the range is not, which is the whole report. Their own docstring cites the requirement and says it *might have security issues* — so the news is the number and the one-line fix (`secrets.token_bytes(8)`). Says out loud that it buys nothing until issue **5** is fixed: a value nobody compares does not have to be hard to guess. **On the wire 2026-08-15**, 24 consecutive sessions against their own station: **24 of 24 eight ASCII digits, 24 distinct**, all inside `[0, 10⁸)` — and read out of `SessionSetupRes` with **no EXI decoder**, because the field sits at a one-bit offset and a single bit shift makes the digits legible as text. That a SessionID can be read out of a hex dump is the shortfall wearing different clothes |
| [`evdriveflow-service-discovery-filter.md`](evdriveflow-service-discovery-filter.md) | EDF Lab (eVDriveFlow) | **3** | `SupportedServiceIDs` is optional — omitting it means *"list everything"* — and their station dereferences it unconditionally, so **every EV that does not pre-filter** dies at the fifth message. In the same three lines, a filter naming neither of their two services leaves the mandatory `EnergyTransferServiceList` unset behind an `OK`. Both are the `Optional`-is-`None` family the report counts: seven `hasattr` sites in four files, on both sides. **Two additions 2026-08-15.** Sending the element shows what the wall is in front of: their station then runs the whole DC sequence to `PowerDelivery`, all `OK` — so this is a working station with one line between it and every conformant car. And the identical unguarded read is in the **charge loop** (`process_dc_charge_loop_request.py:114`, the optional `DisplayParameters`), where it is hit by a session that has done everything right — same pattern, second file, first time observed |
...
| [`everest-isomux.md`](everest-isomux.md) | EVerest (`IsoMux`) | **four**, one per section | Four defects in the one module that fronts both protocols, merged into one report on 2026-08-10 because they are one module and — for three of them — one shape, *a decision taken on information the module does not have or does not read*: the backend is chosen on the **first** `-20` entry and never on `Priority` (`[V2G2-169]`); TLS is capped at 1.2 and `-20` is routed onto it anyway, so the `-20` backend is reachable **only** by a non-conformant EV (`[V2G20-2356]`); a failed V2GTP header read is logged and then ignored, so the routing decision is taken from a buffer that was never filled; and the TLS server boots with **`trusted_ca_keys support disabled`**, the extension `[V2G2-651]` obliges every EV to send. Both backends behind the mux implement the first correctly. **§4 measured further 2026-08-15**: a **second V2G root with a valid chain under it** changes nothing — the two boot lines are identical, so *"then configure a trust anchor"* is closed. **And §4 is decided 2026-08-16, in four arms with the ClientHello on tape**: naming root **B** while trusting only B is refused, naming root **B** while trusting only **A** completes a full DC session, and a control brackets both. The capture shows the car naming one authority (`cert_sha1_hash` of `CN=V2GRootCA-B`) and the mux answering with a chain that verifies under root **A** and fails under B, while `CN=SECCCert-B` sat installed and valid. **The extension arrives and changes nothing** — consequence measured, not inferred from the boot line. It took building `[V2G2-651]` on our side first: §2's TLS 1.2 cap made `openssl -requestCAfile` useless (that is the TLS 1.3 extension), so the client had to be one that speaks RFC 6066 over TLS 1.2, and **that also dissolved the §2/§4 coupling this row used to assert** |
...
| [`everest-d20-sequence-timeout.md`](everest-d20-sequence-timeout.md) | EVerest (`libiso15118`) | one | `V2G_SECC_Sequence_Timeout` is a single 60 s constant armed from one call site, so a silent EV holds the **charge loop** — contactor closed — for a minute where Tables 216/217 give the SECC **0,5 s** (`[V2G20-1500]`, `[V2G20-1502]`). Measured against a normal-charge control **on both paths**, each from their own log: **60,0025 s** after `DcChargeLoopRes` (2026-08-11) and **60,0160 s** after `AcChargeLoopRes` (2026-08-15), agreeing to 14 ms as one shared constant should. The AC arm was run so that Table 216's row is a measurement rather than an inference from Table 217's. The arming, disarming and session-stop around it are all correct — one value, not a broken design. Their log line names a third number again (40 s, the EV's row of Table 215), identically on both paths |
...
| [`everest-evsev2g-paymentdetails-crash.md`](everest-evsev2g-paymentdetails-crash.md) | EVerest (`EvseV2G`) | one | **A malformed contract certificate in `PaymentDetailsReq` crashes the V2G module.** `handle_iso_payment_details` parses the cert at `iso_server.cpp:982`, **uses** it at `:990` (`getEmaidFromContractCert`), and only checks the parse result at `:1006`. On unparseable DER the parse returns a **null** `certificate_ptr`, so line 990 runs `certificate_subject(nullptr)` — which opens `assert(cert != nullptr)` and then `X509_get_subject_name(cert)`: **SIGABRT** in a debug build (their CMake default), **SIGSEGV** in release. Reachable **pre-authentication** — `-2` TLS is unilateral and the crash is during parsing, before any signature check. **Reproduced on a running 2026.02.1 station on 2026-08-15, twice**, from a peer that presented no client certificate: SIGABRT at `openssl_util.cpp:775`, five frames from TCP connect. With a **control** whose certificate is well-formed and untrusted (answered `OK`, station alive) and a **liveness** arm afterwards, which corrected the consequence: the manager does **not** restart the module, it answers with *Terminating all modules* / *Exiting manager* and the next connection is refused. **Not** a controllable write — availability, not code execution — and **no ISO clause**, it stands on the crash. Josev and ours both **survive** the same bytes, differently. One reordered check fixes it. **Every technical box is now ticked**; what is left is a person choosing the disclosure route |
| [`everest-evsev2g-certificate-update.md`](everest-evsev2g-certificate-update.md) | EVerest (`EvseV2G`) | one | `handle_iso_certificate_update` is `// TODO: implement CertificateUpdate handling` and `return V2G_EVENT_NO_EVENT` — the ordinary carry-on value, not `IGNORE_MSG` — so the dispatch **sends the response anyway**. `iso2_BodyType`'s bodies are a **union**, none of the three inits touches its members, and every `*ResType` starts with `ResponseCode`: the answer to a contract-renewal request is therefore the **previous message's** response code, `OK` in any session that got that far, with all five other mandatory elements stale bytes of another type. `[V2G2-556]` makes acting on the request a *shall*, `[V2G2-558]` makes `FAILED` the owed answer, `[V2G2-736]` the schema-conformant fill. Explicitly **not** a memory disclosure — the encoder bounds-checks lengths. **Measured 2026-08-16**: the response came back `ResponseCode = OK` and the receiving car threw on `DHpublickey: expected a 65-byte uncompressed P-256 point`, so of the two outcomes the report named it is the quiet one — it encodes, reaches a car, and claims success. Their Installation path logs a phase and waits 4 500 ms; the Update path logs none and answers in ~134 ms. Reaching it needed a car naming **their** advertised parameter set while sending the other message, which their state table admits and their advertisement does not — an off-profile probe, and the report says so. Bounded by a sweep of **all seventeen** handlers: sixteen assign `ResponseCode` between 2 and 13 times, this one never. Josev does not implement the feature either and answers `FAILED` correctly in nine lines |
| [`everest-evsemanager-renegotiation-supply.md`](everest-evsemanager-renegotiation-supply.md) | EVerest (`EvseManager` + `EvseV2G`) | one | **A DC renegotiation leaves the charger `Inoperative`.** `ChargeProgress = Stop` publishes `current_demand_finished`, which `EvseManager` binds to `powersupply_DC_off()`; `ChargeProgress = Renegotiate` sets a flag `EvseV2G` reads itself and publishes **nothing**, and `grep -rn "enegotiat"` over `modules/EVSE/EvseManager/` matches nothing at all. So the supply keeps serving, and `cable_check()` — which **verifies** the link is below 60 V rather than establishing it — times out on the `CableCheck` their own state machine correctly requires next: `MREC11CableCheckFault` → `Inoperative` → the station stops serving **the following** sessions until restarted. Measured twice, and the second arm exists to rule us out: re-run with `EVReady = false`, byte-identical outcome, because nothing in that path reads the field. **The third instance of one shape** — the layer that is right sits under the layer that decides, after the EIM verdict and the contactor level. Explicitly the correction of a withdrawn neighbour: [`everest-evsev2g-renegotiation-cablecheck`](everest-evsev2g-renegotiation-cablecheck.md) blamed the state machine and was wrong; this report exists because fixing **our** car let the session get one phase further |
| ~~[`everest-evsev2g-renegotiation-cablecheck.md`](everest-evsev2g-renegotiation-cablecheck.md)~~ **WITHDRAWN 2026-08-15** | ~~EVerest (`EvseV2G`)~~ | ~~one~~ | **Their station was right.** The DC SECC state table goes `ChargeParameterDiscovery` → *Wait for CableCheckReq* (`[V2G2-565]`, `[V2G2-582]`) with no renegotiation exception, so `FAILED_SequenceError` is the correct answer to a `PowerDeliveryReq` that skipped `CableCheck` and `PreCharge` — and it is **our** car that skips them. The report leaned on Annex I, whose two diagrams carry `ChargingStatusReq/Res` and are therefore **AC**, where there is no CableCheck to skip. Withdrawn by working its own unticked gate (*"check the 2014 wording before posting"*) rather than by anything new arriving. The text below is the original: **A DC renegotiation is accepted and then cannot restart.** `PowerDeliveryReq(Renegotiate)` and the fresh `ChargeParameterDiscovery` are both answered `OK`; the `PowerDeliveryReq(Start)` that resumes the charge — the message `[V2G2-842]` names as the *next following* one — gets `FAILED_SequenceError`. `handle_iso_charge_parameter_discovery` puts a DC session into `WAIT_FOR_CABLECHECK`, whose mask admits `CableCheck` and `SessionStop` only, so their station wants the isolation test re-run. **Annex I's own sequence** goes `Renegotiate → ChargeParameterDiscovery → PowerDelivery → charging loop`, and the NOTE at `[V2G2-680]` says the contactor stays closed throughout — CableCheck is the test performed *before* it closes. The branch that would have accepted the restart already exists next door, gated on a pause flag. Measured on the wire against their stock DC SIL, no PKI and no TLS. **Carries the `-2` document caveat harder than most**: every quotation is from the 2022 DIS and the checklist says to re-read the 2014 wording first |
...
| [`josev-iso20-charge-loop-timeout.md`](josev-iso20-charge-loop-timeout.md) | SwitchEV (iso15118) | one | `V2G_SECC_SEQUENCE_TIMEOUT_{AC,DC,WPT}_CL = 0.5` are transcribed from Tables 216/217 into their own timeouts file and **referenced nowhere**; both charge-loop states hand on the 60 s baseline instead, so a silent EV holds the loop — contactor closed — for a minute where `[V2G20-1500]`/`[V2G20-1502]` give 0,5 s. Two lines to fix, and the constants already exist. **Measured on their SECC 2026-08-15**, four arms: **60,061 s** after `DC_ChargeLoopRes` and **60,060 s** after `AC_ChargeLoopRes`, with **their own log naming the value** — *"Waited for 60.0 s"*, printed from `DCChargeLoop` and `ACChargeLoop`. Both modes run because the fix is two lines and neither should rest on the other. The identical behaviour was measured against EVerest's implementation, so two independent `-20` stacks flatten the same override and all four intervals land within 60 ms. **Running it cost two harness fixes of ours first** — the fixture passed four of eleven parameters, so the first silent arm produced a complete session, and it dropped the power mode before the SAP handshake, so AC could not negotiate at all |
| [`josev-iso20-evcc-charge-loop-pacing.md`](josev-iso20-evcc-charge-loop-pacing.md) | EVerest (`PyEvJosev`) — **re-aimed 2026-08-15**, it was addressed to SwitchEV | one | The other side of the same table, and the only filing here about a **performance** criterion. Their `-20` EVCC turns the AC charge loop around in **≈532 ms** where Table 216 gives the EVCC **0,25 s** (`[V2G20-1499]`) — measured against our station's own conformant 0,5 s timer, which arms after our response is written and so measures exactly Figure 212's span: **2 of 2 strict runs die on the *first* charge loop**, and relaxing the timer to 20 s gives 44 pairs at ≈532 ms each from their own log. `V2G_EVCC_SEQUENCE_PERFORMANCE_TIME` is **not in `timeouts.py` at all** — a different shape from the SECC constant next door that exists and is unreferenced. **Written carefully as a deviation, not a violated timeout**: Figure 212's legend sorts the EVCC's threshold as *performance* and the SECC's as *error*, and there is no `[V2G20-443]`-shaped clause aimed at the car — the consequence lands at the station, where an EV above 0,5 s simply cannot charge. Not a charge-loop pacing decision either: the setup phase costs ≈573–600 ms per exchange, so it is the session's per-message cost. **Localized 2026-08-15, and it changed the addressee.** Not the codec — their own three log lines split each exchange into decode ≈30 ms, handling ≈1 ms, encode ≈30 ms, across four sessions and both roles, so the py4j round-trip §4 suspected first is an order of magnitude short. Not the EVCC — **Josev's own container turns the same charge loop around in 43 ms** in the identical scenario, twelve times faster than through `PyEvJosev`. The remaining 480 ms is not localized further, because their module clears the root logger and never sets a level, so the split cannot be taken inside it without a patch that changes what is measured — and the report hands over that sentence rather than a contaminated figure. Only visible because a conformant station was on the other end — their own `-20` station and this EVCC's own SECC both wait 60 s |
| [`josev-iso20-vehicle-certificate.md`](josev-iso20-vehicle-certificate.md) | SwitchEV (iso15118) | one | Their EVCC presents the **OEM provisioning** certificate as its TLS client credential, because the stack has no vehicle certificate: `grep -c VEHICLE shared/security.py` → **0**, nothing matches `vehicle_cert`/`VEHICLE_LEAF` under `iso15118/`, and `create_certs.sh` mints no branch — while the same leaf is also the `OEMProvisioningCert` of `CertificateInstallationReq`. `[V2G20-2339]` and `[V2G20-2342]` are two *shall*s for two credentials, clause 7.3.1 separates them, and `[V2G20-2598]` puts the **EVCCID** in the vehicle certificate's Common Name — where the measured leaf reads `CN=OEMProvCert`, carries two RDNs that requirement excludes, and has no `ExtendedKeyUsage` where Annex B.8 puts `id-kp-clientAuth`. **Nothing observably breaks** — our station completed a full DC session on it — and the report leads with that; the cost is `[V2G20-2677]`, which binds pause/resume to whatever the handshake presented. **The fix is a diff that exists**: their own downstream fork `26f7988` carries the whole `VEHICLE_*` path set and mints `CN=WMIV1234567890ABCDEX, O=Pionix` under the V2G root. A second half in the same function: `:171` verifies against the OEM root alone, where `[V2G20-2331]` also permits a V2G-rooted vehicle chain |
...
**Forty-nine filings written across six projects — three of them withdrawn**, and four of the rest have
to be sent **twice**.
...
**That "thirty-two" is the file count on 2026-08-11 and is not the "thirty-four" above.** There are
**thirty-seven** report files now — the forty-fifth to forty-ninth filings were written on
08-12, 08-13, 08-14, 08-15 and 08-16 — of which **three are withdrawn** and thirty-four are sendable. The
arithmetic is written out here so nobody has to rediscover it: 37 files − 3 withdrawn = 34 sendable,
carrying 48 issues. (On 2026-08-11 the file count and the sendable count were both thirty-two, which was
a coincidence and is no longer one.)

**The third withdrawal is a different kind from the first two, and the difference is the point.** Those
two were retired because everest-core fixed them independently — evidence that the findings were real.
This one was retired because **it was never right**: their station implements ISO 15118-2's DC state
table and our reading of Annex I did not survive being checked. Nobody moved; a checklist item that had
been sitting unticked since 2026-08-11 was worked, and it took the report with it. That is the strongest
argument these checklists have — and it is also the shape of the warning, since the item said *"this
project cannot"* about a check that turned out to be available all along.
