...
| **The EVCC half of the same two table rows, and this one was measured** | `shared/messages/iso15118_20/timeouts.py` has **no `V2G_EVCC_SEQUENCE_PERFORMANCE_TIME` at all** — the file transcribes the four EVCC timeouts of Table 215 and the per-message `V2G_EVCC_Msg_Timeout` values (`AC_CHARGE_LOOP_REQ = 0.5`, correct), and `grep -rn PERFORMANCE iso15118/` finds only an unrelated local constant in the SECC simulator. So where the row above is a value written down and never referenced, this is a parameter that was never transcribed. **Measured 2026-08-13** through EVerest's `PyEvJosev` against our SECC: their EVCC turns the AC charge loop around in **≈532 ms** where Table 216 gives it **0,25 s** (`[V2G20-1499]`), and 2 of 2 strict runs died on the *first* charge-loop pair against our conformant 0,5 s timer, which arms after our response is written and therefore measures exactly the interval Figure 212 puts both thresholds on. **Not a charge-loop pacing decision** — the setup phase costs ≈573–600 ms per exchange in the same logs, so it is the session's per-message cost; the only live sleep on the EVCC path is the 0,1 s SDP start-up sync in `evcc/comm_session_handler.py`, and `continue_charging`'s `asyncio.sleep(0.5)` is commented out. **Localized 2026-08-15 — and it is not theirs.** The codec is exonerated by their own three log lines (decode ≈30 ms, handling ≈1 ms, encode ≈30 ms, four sessions, both roles), and **Josev's own EVCC container turns the same AC charge loop around in a median of 43 ms** against the same station in the identical scenario — twelve times faster than through EVerest's `PyEvJosev`. So the ≈532 ms belongs to the module that wraps this stack, and [the filing was re-aimed at EVerest](reports/josev-iso20-evcc-charge-loop-pacing.md); what stays upstream's is the absent constant in this row and nothing else. Filed 2026-08-14, re-aimed 2026-08-15: [run](interop-runs/2026-08-13-everest-d20-ac-reverse/notes.md), [localization](interop-runs/2026-08-15-josev-evcc-pacing-localized/notes.md). One correction to the sentence above while we were there: the *fork's* only live EVCC sleep is the 0,1 s SDP sync, but **upstream has a real charge-loop delay hook** the fork lacks — `charge_loop_delay()` → `asyncio.sleep(delay)`, defaulting to 0 |
...
| **Checked — a malformed contract cert is survived, but not answered `FAILED`** | Corrected on a re-read of the exception paths. A parseable cert that fails *verification* gets a proper `FAILED` from the state's own `except` (`iso15118_2_states.py:~1058`). An **unparseable** one raises `ValueError` at `load_der_x509_certificate` (top of the `try:`, via `log_certs_details`), which that `except` does **not** list; it falls through to the framework rcv-loop catch-all `except (… ValueError, … Exception)` (`shared/comm_session.py:510-516`) → `self.stop(…)` (`:543`), **terminating the session with no `PaymentDetailsRes`**. So Josev **survives** where EVerest's `EvseV2G` [crashes](reports/everest-evsev2g-paymentdetails-crash.md) on the same bytes — but does not send `FAILED_CertChainError` either, and neither does our own station (it returns `OK` and fails a message later). The filable crash is EVerest's alone; the shared miss of `FAILED` for the unparseable case is a smaller note, one of whose three is us |
...
| **Checked and found correct in the part that matters — their EVCC's TLS client credential is anchored where `-20` puts it** | `shared/security.py:209` loads `CertPath.OEM_CERT_CHAIN_PEM` + `KeyPath.OEM_LEAF_PEM` into the client context whenever `ENABLE_TLS_1_3` is set, and their station verifies it against `CertPath.OEM_ROOT_PEM` (`:171`). **Measured 2026-08-15**, not merely read: with our station's store configured for the first time in this direction, their car's chain validates and anchors at `CN=OEMRootCA`. That is the class `[V2G20-2331]` prescribes (OEM root, V2G root as the permitted alternative) and the pairing clause 7.3.1 draws, and the **exact inverse** of the EVerest station that took a *contract* certificate for the job ([filing](reports/everest-d20-trust-anchor.md)) — worth recording because it shows the requirement is implementable and someone implemented it. **The leaf is a defect, and it was audited the same day.** What their car presents is `CN=OEMProvCert`, the OEM *provisioning* certificate: `[V2G20-2339]` makes a **vehicle** certificate the TLS credential and `[V2G20-2342]` makes the provisioning certificate a separate credential for contract installation, clause 7.3.1 separates them, and `[V2G20-2598]` requires the EVCCID as the vehicle certificate's Common Name — this one names a role, carries two RDNs the requirement excludes, and has no `ExtendedKeyUsage` where Annex B.8 puts `id-kp-clientAuth`. `grep -c VEHICLE shared/security.py` → **0**, and nothing matches `vehicle_cert`/`VEHICLE_LEAF` anywhere under `iso15118/`: the class is absent from the stack, and the same leaf is also the `OEMProvisioningCert` of `CertificateInstallationReq` (`evcc/states/iso15118_20_states.py:198`). **Their own downstream fork already implements it** — `26f7988` has a full `VEHICLE_*` path set and mints `CN=WMIV1234567890ABCDEX, O=Pionix` under the V2G root — so the ask is a port, not a design. Filed 2026-08-15: [`josev-iso20-vehicle-certificate.md`](reports/josev-iso20-vehicle-certificate.md), [audit](interop-runs/2026-08-15-josev-tls-vehicle-cert-audit/notes.md). A second half in the same function: `security.py:171` verifies against the OEM root only, where `[V2G20-2331]` also permits a V2G-rooted vehicle chain — which is exactly what the fork mints |
