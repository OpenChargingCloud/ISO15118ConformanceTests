...
| `←SECC` AC ×2, **Porsche Taycan 4S** (both sides) | their `pcap-iso15118` on the two Taycan captures, folded and unfolded | **A finding against us**: the car's profile asks for 11,040 W — 3 × 230 V × 16 A — against our rounded 11,000 W offer, so [V2G2-761] refuses `PowerDelivery`. Fixed the same day (the offer is now the physical number); the re-run against their injector is owed. Unfolded, both routes confirm the sequence-guard fix on a second and third real car |
