...
[`2026-08-06-tux-tls`](interop-runs/2026-08-06-tux-tls/notes.md).
...
| `←SECC` AC ×2, **Porsche Taycan 4S** (both sides) | their `pcap-iso15118` on the two Taycan captures, folded and unfolded | **A finding against us**: the car's profile asks for 11,040 W — 3 × 230 V × 16 A — against our rounded 11,000 W offer, so [V2G2-761] refused `PowerDelivery`. Fixed and re-run the same day: both captures now reach `SessionStop`, 10 exchanges all `OK`, their TAP 12/12, the flow matching their declared order exactly. Unfolded, both routes confirm the sequence-guard fix on a second and third real car, and re-running them after the fix changes nothing — the session ends four messages earlier |
...
One smaller thing their decoder made visible, ✅ **fixed the same day**: our `SessionSetupRes` carried a
**hardcoded** `EVSETimeStamp` of `1600000000` — 13 September 2020 — in every session this station ever
answered, although `Secc2` holds a clock and used it correctly two messages later (`PaymentDetailsRes`,
the metering receipts). It now reads the clock, which an EV without one of its own takes as the time
([V2G2-748]).

Worth noting *why* nothing here caught it, because it is the shape of finding this project keeps
meeting (`docs/assumed-values-sweep.md`): a station may legitimately report any time, so no conformance
check could object, and our own EV ignores the field — the value was invisible to every oracle in this
repository and needed a foreign decoder that simply prints the date. The fix needed no new recording
seam: the corpus already drives a `ManualTimeProvider`, which is why those two other messages had always
regenerated byte-identically.

---

## The TLS layer, and the two walls in it

The [TLS run](interop-runs/2026-08-06-tux-tls/notes.md) produced the sharpest finding this
counterparty has yielded, and it is not about them alone.

**Their cipher-suite profile contains neither suite ISO 15118-2 prescribes.** Both shipped configs pin
the GnuTLS priority string `SECURE128:-VERS-SSL3.0:-VERS-TLS1.0:-ARCFOUR-128:+PSK:+DHE-PSK`; its ECDSA
half is AES-GCM, AES-CCM, ChaCha20 and two SHA-1 CBC suites, and **`…AES_128_CBC_SHA256` is absent in
both the ECDHE and the ECDH form**. Our station pins exactly those two, so the handshake ends in
`no shared cipher` — a station implementing the standard's TLS profile and a car using their
configuration cannot connect at all. Their own pair interoperates because both ends share the same
non-conformant profile. Worth stating without blame: GnuTLS's curated `SECURE128` dropped the SHA-256
CBC suites as legacy, so any stack reaching for a modern curated list loses precisely what a 2014
standard made mandatory.

**Their EVCC signs the `AuthorizationReq` whenever a `pki` block is configured** — which, over TLS,
is always (`exi-15118/src/net-exi.rs:187`, keyed on config presence rather than on the session having
selected Contract). The captured Audi scenario is EIM, so no `PaymentDetailsRes` ever issued a
challenge, and their signer fails with `no_challenge` four exchanges in. Their own responder
reproduces it exactly, which means their README's TLS quick-start cannot succeed as written and no
shipped scenario of theirs runs over TLS today.
...
recorded answer. Then TLS, the same day: their PKI, a session over TLS 1.2 with mutual authentication
available, and **two walls that are theirs** — a cipher-suite profile without either suite the standard
prescribes, and an EVCC that signs an EIM authorization because TLS is configured.
...
arm of our state machine no self-consistent test had ever executed — and one layer down, the first
external check of our TLS profile found a peer that cannot satisfy it, which no oracle made of our own
parts could ever have reported.
