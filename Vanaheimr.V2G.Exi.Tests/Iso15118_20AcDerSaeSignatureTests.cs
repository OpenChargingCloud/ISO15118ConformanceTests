...
using Vanaheimr.V2G.Iso15118_20.AC_DER_SAE;
using Vanaheimr.V2G.Iso15118_20.AC_DER_SAE.Generated;

namespace Vanaheimr.V2G.Exi.Tests
{
    /// <summary>
    /// ISO 15118-20 AC_DER_SAE XMLDSig signing/verification: same ECDSA-P521/SHA-512 suite as
    /// CommonMessages' (<see cref="Iso15118_20SignatureTests"/>), against this set's own
    /// <c>SignedInfo</c>/<c>V2GSignature</c>.
    /// </summary>
    /// <remarks>
    /// Unlike the AC and DC fixtures, the fragment octets underneath are <b>not</b> pinned to a
    /// reference encoder: cbexigen does not generate the amendment schemas. What is asserted here is
    /// that the suite works over this set's own grammar — never that those are the right bytes.
    /// </remarks>
    [TestFixture]
    public class Iso15118_20AcDerSaeSignatureTests
    {
        private static ReferenceType SignedElementReference(out byte[] fragment)
        {
            ...
            Assert.That(AcDerSaeCodec.EncodeFragment_AC_ChargeParameterDiscoveryRes(content, buf, out int n), Is.True);
