...
/// <summary>
/// Differential wire conformance for the ISO 15118-20 DC EXI fragment codec. cbV2G's
/// <c>iso20_dc_exiFragment</c> carries exactly two signable elements
/// (<c>include/cbv2g/iso_20/iso20_DC_Datatypes.h</c>): <c>SignedInfo</c> and
/// <c>DC_ChargeParameterDiscoveryRes</c>. Diffed against <c>cbv2g_iso20 Fragment_DC_&lt;name&gt;</c>
/// (tools/cbv2g-ref/main_iso20.c, <c>do_fragment_dc</c>).
/// </summary>
[TestFixture]
public class Iso15118_20DcFragmentTests
{
    [Test]
    public void SignedInfo_Fragment_MatchesCbV2G()
    {
        ...
        Assert.That(DcCodec.EncodeFragment_SignedInfo(content, buf, out int n), Is.True);
        AssertFragment(
            "80 81 44 ad 0e 8e 8e 07 45 e5 ee ee ee e5 ce e6 65 cd ee 4c e5 ea 8a 45 ec 6c 2d cd ed cd " +
            ...
            "51 59 61 69 71 79 81 89 91 99 a1 a9 b1 b9 c1 c9 d1 d9 e1 e9 f1 fa 00 cb 00",
    ...
    {
        var content = new DC_ChargeParameterDiscoveryResType(
            Header: new MessageHeaderType(SessionID: new byte[8], TimeStamp: 1_700_000_000UL, Signature: null),
            ResponseCode: ResponseCode.OK,
            DC_CPDResEnergyTransferMode: new DC_CPDResEnergyTransferModeType(
                EVSEMaximumChargePower: new RationalNumberType(1, 15000),
                EVSEMinimumChargePower: new RationalNumberType(0, 100),
                EVSEMaximumChargeCurrent: new RationalNumberType(0, 200),
                EVSEMinimumChargeCurrent: new RationalNumberType(0, 1),
                EVSEMaximumVoltage: new RationalNumberType(0, 500),
                EVSEMinimumVoltage: new RationalNumberType(0, 200),
                EVSEPowerRampLimitation: null));
        var buf = new byte[512];
        Assert.That(DcCodec.EncodeFragment_DC_ChargeParameterDiscoveryRes(content, buf, out int n), Is.True);
        AssertFragment(
            "80 1a 01 00 00 00 00 00 00 00 00 02 03 8b 3e a8 18 80 11 02 13 0e a0 80 06 40 40 06 40 08 " +
            "20 00 04 10 01 e8 06 08 00 c8 01 14 b0",
            buf.AsSpan(0, n).ToArray());
    }
